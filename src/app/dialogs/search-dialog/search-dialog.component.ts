  ...
  async onSubmit() {
    ...
    if (this.searchType === 'Email') {
      try {
        ...
        if (err.error && err.error.message && err.error.message === 'User not found') {
          this.snackbarService.showError(err.error.message, 'Close');
        }
        console.log(err);
        this.loading = false;
        return;
    ...
    try {
      ...
      if (!user) {
        this.snackbarService.showError('Invalid Uid', 'Close');
      }
      console.log(user);
